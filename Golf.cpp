...
ll linearSearch(ll arr[], ll n, ll ele)
{
    FOR(i,n)
    {
        if(arr[i]==ele)
...
ll binarySearch(ll arr[], ll n, ll ele){
    ll low=0,high=n-1;
    while(low<=high){
        ll mid=(low+high)>>1;
        if(arr[mid]==ele)
...
void dfs(vector<vector<ll> > &graph, ll src ,vector<bool> &visited)
{
    visited[src]=true;
...
void bfs(vector<vector<ll> > &graph, ll src,ll n){
    queue<ll> q;
