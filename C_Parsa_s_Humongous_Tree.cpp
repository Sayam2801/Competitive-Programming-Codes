...
void dfs(ll src,ll parent)
{
    ans[src].f=0;
    ans[src].s=0;
    for(auto x:adjacent[src])
    {
        if(x!=parent)
        {
            dfs(x,src);
            ll pos1=ans[x].f+abs(a[src].f-a[x].f);
            ll pos2=ans[x].s+abs(a[src].f-a[x].s);
            ll pos3=ans[x].f+abs(a[src].s-a[x].f);
            ll pos4=ans[x].s+abs(a[src].s-a[x].s);
            ans[src].f+=max(pos1,pos2);
            ans[src].s+=max(pos3,pos4);
        }
    }
    return;
}
