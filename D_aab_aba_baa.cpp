...
//     If there are more than or equal to K
//     strings which starts with a, then it is a;
//     otherwise, it is b.
// For each 0≤i≤A,0≤j≤B
// , precalculate C(i,j), the total number of strings consisting with i number of a’s and j number of b’s. Since it is equal the number of ways to travel from (0,0) to (i,j) by repeating advancing in the positive direction of x or y axis by 1

// , so it can be calculated with a simple DP (Dynamic Programming).

// Let S(i,j,k)
// be “the k-th string in the lexicographical order among the strings consisting of i a’s and j

// b’s.” This can be found by splitting into the following cases:
...
// , a followed by S(i−1,j,k)
// ;
// If C(i−1,j)<k
// , b followed by S(i,j−1,k−C(i−1,j)).
const int MAX=30;
ll dp[MAX+1][MAX+1];
string findKthLexicographicString(ll a,ll b,ll k)
{
    if(!a)
       return string(b,'b');
    if(!b)
       return string(a,'a');
    if(k<=dp[a-1][b])
       return string("a") + findKthLexicographicString(a-1,b,k);
    else
       return string("b") + findKthLexicographicString(a,b-1,k-dp[a-1][b]);
}
void solve(){
    ll a,b,k;
    cin>>a>>b>>k;
    ...
    cout<<findKthLexicographicString(a,b,k)<<endl;
    return;
}
int main()
{
