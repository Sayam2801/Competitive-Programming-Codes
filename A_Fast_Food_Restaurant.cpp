...
void solve(){
    int a,b,c,ans=0;
    cin>>a>>b>>c;
    ans=max(ans,distribute(a,b,c));
    ans=max(ans,distribute(a,c,b));
    ans=max(ans,distribute(b,a,c));
    ans=max(ans,distribute(c,a,b));
    ans=max(ans,distribute(b,c,a));
    ans=max(ans,distribute(c,b,a));
    cout<<ans<<endl;
    return;
}
int main()
