...
/* EDITORIAL: For each cell (i,j), let's associate it with the color (i+j)mod3

. Every three consecutive cells contains one of each color. So, if we choose one color and flip all tokens with that color, it will be a solution. We need only prove that there is a color associated with at most one third of the tokens.

Let there be xi
tokens in cells of diagonal number i, for i=0,1,2. Then we have k=x0+x1+x2. Therefore, min{x0,x1,x2}≤⌊k3⌋

.

Here's the coloring on an example. There are 3
X's on red, 4 on blue, and 5 on green. Red has the fewest, so we flip those. */
const int N=305;
string s[N];
void solve(){
    int n;
    ...
    int temp=min_element(cnt,cnt+3)-cnt;
    FOR(i,n)
    {
        FOR(j,n)
        {
            if(s[i][j]=='X' && (i+j)%3==temp)
                s[i][j]='O';
        }
        cout<<s[i]<<endl;
    }
