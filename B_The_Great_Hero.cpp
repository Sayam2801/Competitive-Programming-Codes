...
void solve(){
    ...
    while(i<n)
    {
        ll round_to_destroy_monster=(ll)(monster[i].s+attack-1)/attack;
        ll round_to_destroy_hero=(ll)(health+monster[i].f-1)/monster[i].f;
        if(round_to_destroy_hero<round_to_destroy_monster)
        {
            cout<<"NO"<<endl;
            return;
        }
        health-=round_to_destroy_monster*monster[i].f;
        i++;
        if(health<=0)
           break;
    }
