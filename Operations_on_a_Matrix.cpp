...
//Tutorial: First, let's observe that we can replace array ai with array bi = ai  -  ai - 1, because all we care about are differences between neighboring elements. Now, we can see that our lost array can have length d if and only if for every j such that j  +  d  ≤  n, bj  =  bj + d. So we can iterate over every possible d from 1 to n and check if it is correct in O(n). Complexity of whole algorithm is O(n2).
//In this problem, only making variables global made all the difference, this is how coding usually gets messed up quite a lot..
ll n;
ll arr[1001];
bool isPossible(ll pos)
{
    for(ll i=0;i+pos<n;i++)
        if((arr[i+1]-arr[i])!=(arr[i+pos+1]-arr[i+pos]))
           return false;
    return true;
}
ll row[1000005],col[1000005];
void solve(){
  ll n,m,q;
  cin>>n>>m>>q;
  FORL(i,q)
