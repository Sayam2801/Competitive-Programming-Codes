...
void solve(){
    char grid[3][3];
    int cntx=0,cnto=0,cntu=0,winx=0,wino=0;
    FOR(i,3)
    {
        FOR(j,3)
        {
    ...
    //Checking along the rows of the grid
    FOR(i,3)
    {
    ...
    //Checking along the columns of the grid
    FOR(i,3)
    {
    ...
    //Checking along the diagonals of the grid
    ...
    //Checking along opposite diagonals of the grid
    ...
    if((cnto>cntx) || (cntx-cnto>1))
       cout<<3<<endl;
    else if(((cntx>cnto) && winx && !wino) || ((cntx==cnto) && wino && !winx))
        cout<<1<<endl;//At the last step, either 'X' or 'O' can win 
    else if(!cntu && !(winx+wino))
        cout<<1<<endl;//If no empty space is left and neither of 'X' or 'O' have won yet, then it means it's a draw
    else if(!cntu && (winx==2))
        cout<<1<<endl;//Special case when X has two possible positions to win the game
    else if(cntu && !(winx+wino))
        cout<<2<<endl;//If a few more moves are still left and 'X' and 'O' haven't won yet, then it means that game can continue further... 
    else 
        cout<<3<<endl;//Other unreachable states 
    return;
}
int main()
{
