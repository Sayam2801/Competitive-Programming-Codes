...
void solve(){
    ...
    cin>>n;
    vl arr(n);
    vector<ll> from={0},to={0};
    FOR(i,n) 
    {
       cin>>arr[i];
       if(arr[i])
          from.pb(i); //Contains indexes of occupied positions 
       else
          to.pb(i); //Contains indexes of unoccupied positions
    }
    ll s1=sz(from),s2=sz(to);
    if(s1==1 || s2==1)
       cout<<0<<endl;
    else  
    {
        vvl dp(s1,vl(s2,INF));
        //If none of tbe positions are occupied, then ans will always be 0   
        for(ll i=0;i<s2;i++)
            dp[0][i]=0;
        for(ll i=1;i<s1;i++)
        {
