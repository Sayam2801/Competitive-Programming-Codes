...
void solve(){
   ...
   FOR(i,n)
    ...
    if(!check)//If there will be no zero present in the array, then ans will be -1 as regardless of whichever no we can form from the array, we can't form any number divisible by 90 in the absence of zero..
    {
        cout<<-1<<endl;
        return;
    }
