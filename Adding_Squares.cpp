...
void solve(){
    ll w,h,n,m;
    cin>>w>>h>>n>>m;
    bitset<mx> vertical,horizontal,reverseHorizontal;
    FORL(i,n){
       ll no;
       cin>>no;
       vertical.set(no);
    ...
    FORL(i,m){
       ...
       cin>>no;
       horizontal.set(no);
       reverseHorizontal.set(h-no);
    }
    bitset<mx> verticalDiff,horizontalDiff;
    ll maxSqaures=(ll)INT_MIN;
    for(ll i=0;i<=w;i++)
    {
        if(vertical[i])
            verticalDiff |= (vertical>>i);
    }
    for(ll i=0;i<=h;i++)
    {
        if(horizontal[i])
            horizontalDiff |= (horizontal>>i);
    }
    for(ll i=0;i<=h;i++)
    {
        bitset<mx> newHorizontalDiff;
        newHorizontalDiff |= (horizontal>>i);
        newHorizontalDiff |= (reverseHorizontal>>(h-i));
        ll numOfSq=(verticalDiff & (horizontalDiff | newHorizontalDiff)).count();
        maxSqaures=max(maxSqaures,numOfSq);
    }
    cout<<maxSqaures-1<<endl;
    return;
}
int main()
{
