...
int getMaximum(int arr[],int l,int r)
{
    int max_ele=INT_MIN,x;
    for(int i=l;i<=r;i++)
    {
        if(max_ele<arr[i])
        {
...
void calculateDepth(int arr[],int ans[],int l,int r,int depth)
{
    if(l>r)
        return;
    int mid=getMaximum(arr,l,r);
    ans[mid]=depth;
    depth++;
    calculateDepth(arr,ans,l,mid-1,depth);
    calculateDepth(arr,ans,mid+1,r,depth);
    return;
}
void solve(){
    int n;
    ...
    FOR(i,n)
       cin>>arr[i];
    int ans[n]={},low=0,high=n-1,depth=0;
    calculateDepth(arr,ans,low,high,depth);
    FOR(i,n)
       cout<<ans[i]<<" ";
    cout<<endl;
    return;
