...
void generatePermutations(string s,int startIndex,int endIndex,set<string> &myset)
{
    ...
    for(int i=startIndex;i<endIndex;i++)
    {
        ...
        generatePermutations(s,startIndex+1,endIndex,myset);
        swap(s[startIndex],s[i]);//Backtracking step, in case the permutation generated has been already generated before, then it's not right to make changes for it, we need to restore the positions of characters by reswapping them..
    }
    return;
}
void solve(){
     ...
     int n=s.length();
     set<string> myset;
     generatePermutations(s,0,n,myset);
