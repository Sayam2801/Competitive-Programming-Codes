...
// Let's consider a dynamic programming solution. Let dp[i][k]
// be the maximum possible health achievable if we consider only the first i potions, and k

// is the total number of potions taken.

// The transition is as follows: dp[i][k]=max(dp[i−1][k−1]+ai,dp[i−1][k])
// if dp[i−1][k−1]+ai≥0, and just dp[i−1][k] otherwise. The first term represents the case if we take potion i and the second term is when we ignore potion i

// .

// This runs in O(n2)
// and passes the easy version.
void solve(){
    ll n;
    cin>>n;
    vl arr(n);
    ...
    for(ll i=1;i<=n;i++)
    {
        for(ll j=1;j<=i;j++)
        {
            dp[i][j]=dp[i-1][j];
            if(dp[i-1][j-1]>=0)
               dp[i][j]=max(dp[i-1][j-1]+arr[i-1],dp[i][j]);
        }
    }
    ll ans=0;
    rep(i,1,n+1)
