...
ll getSuperParent(ll x,ll parent[])
{
    return (x==parent[x])?x:parent[x]=getSuperParent(parent[x],parent);
}
void solve(){
    ll n,m,ans=0;
    cin>>n>>m;
    ...
    for(ll i=1;i<=m;i++)
    {
        ...
        if(x==y)
           continue;
        ans++;
        if(getSuperParent(x,parent)==getSuperParent(y,parent))
           ans++;
        else 
           parent[getSuperParent(x,parent)]=getSuperParent(y,parent);
    }
    cout<<ans<<endl;
    return;
}
