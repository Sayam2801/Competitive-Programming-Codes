...
ll uniquePalindromes(string s,int x)
{
    if(x>s.length())
       return 0;//If the required length of palindromic string is greater than length of given string, then no answer is possible   
    int freq[26];//Make a frequency array to count frequency of all 26 characters 
    mem0(freq);
    FOR(i,s.length())
       freq[s[i]-'a']++;//Count frequency of each character in the string
    multiset<int> myset;
    FOR(i,26)
    {
        if(freq[i])
           myset.i(freq[i]);//Store frequency of each character
    }
    ll ans=1;
    FOR(i,x/2)
    {
        ...
        for(auto ele:myset)
        {
            //Check the frequency which is greater than 0
            if(ele>=2)
               cnt++;//No of different character we can place at i and x-i
        }
        if(!cnt)
           return 0;
        else
        ...
        myset.erase(p);
        if(val>2)
           myset.insert(val-2);//Decrease the value of character we place at i and n-i
    }
    if(x&1)
    {
        ll cnt=0;
        ...
        {
            if(ele)
               cnt++;//No of unique characters we can place at position x/2 of odd length palindromic string
        }
        ans*=cnt;
    }
    return ans;
