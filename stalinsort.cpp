...
void solve(){
    ...
    FORL(i,n)
       cin>>arr[i];
    ll j=0;//Starting from the first index
    while(true)
    {
        ll moves=0;
        for(ll i=0;i<(n-1-j);i++)
        {
            if(arr[i]>arr[i+1])//In case of wrong position
            {
                vector<ll>::iterator index;
                ll temp=arr[i+1];
                index=arr.begin()+i+1;//Iterator pointing to the wrongly placed element
                arr.erase(index);//Erase the wrongly placed element
                arr.insert(arr.begin()+moves,temp);//Insert wrongly placed element at the beginning of the array
