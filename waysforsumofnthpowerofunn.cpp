...
//Problem Link: https://www.geeksforgeeks.org/find-ways-integer-can-expressed-sum-n-th-power-unique-natural-numbers/
ll power(ll x,ll n)
{
    if(!x)
       return 0;
...
ll checkRecursive(ll n,ll k,ll curr_ele,ll curr_sum)
{
    ll res=0;
    ll x=power(curr_ele,k);
    while(x+curr_sum<n)
    {
        res+=checkRecursive(n,k,curr_ele+1,curr_sum+x);
        curr_ele++;
        x=power(curr_ele,k);
    }
    if(x+curr_sum==n)
...
void solve(){
    ll n,x;
    cin>>n>>x;
    cout<<checkRecursive(n,x,1,0)<<endl;
    return;
}
int main()
{
